...
const sectionObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            const cards = entry.target.querySelectorAll('.about-block, .it-block, .project-showcase, .tech-item, .personality-block, .entertainment-block, .achievement-category, .contact-card, .scattered-photo, .song-card, .game-item, .diploma-item');
...
dynamicStyles.textContent = `
    @keyframes glitch {
        ...
        20% { transform: translate(-10px, 5px); filter: hue-rotate(90deg) contrast(1.5); }
        40% { transform: translate(-5px, -10px); filter: hue-rotate(180deg) contrast(0.8); }
        60% { transform: translate(10px, 5px); filter: hue-rotate(270deg) contrast(1.3); }
        80% { transform: translate(5px, -5px); filter: hue-rotate(360deg) contrast(1); }
    ...
    .about-block:not(.card-visible),
    .it-block:not(.card-visible),
    .project-showcase:not(.card-visible),
    .tech-item:not(.card-visible),
    .personality-block:not(.card-visible),
    .entertainment-block:not(.card-visible),
    .achievement-category:not(.card-visible),
    .contact-card:not(.card-visible),
    .scattered-photo:not(.card-visible),
    .song-card:not(.card-visible),
    .game-item:not(.card-visible),
    .diploma-item:not(.card-visible) {
        opacity: 0;
    }
`;
